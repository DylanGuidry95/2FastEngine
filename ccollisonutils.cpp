...
float ShapeVsShape(Shape  a, Shape b)
{
	...
	if (a.type == Shape::ePlane && b.type == Shape::eCircle)
		return CircleVsPlane(a.plane, b.circle);
...
float RectVsRect( Rect a,  Rect b)
{
	...
	if (a.min.x > b.max.x || b.min.x > a.max.x)
		return -1; //not colliding in the x
	if (a.min.y > b.max.y || b.min.y > a.max.y)
		return -1; //not colliding in the y
...
{
	vec3 rc = (r.min + r.max) * (0.5f) + r.position;
	...
	p.x = clamp(-dim.x, dim.x, p.x);
	p.y = clamp(-dim.y, dim.y, p.y);
	p.z = 0;


	float t = c.radius - vec3Length((p+rc) - c.position);
	if (t > 0)
	{
		t = t;
	}
...
Shape MakeRect(float x, float y, float xMin, float yMax, float xMax, float yMin)
{
	...
	a.rect.min = vec3Make(xMin, yMax, 0) + a.rect.position;
	a.rect.max = vec3Make(xMax, yMin, 0) + a.rect.position;
	return a;
}
